...
assert_managed_block() {
  ...
  [[ "$begin_count" == "1" ]] ||
    fail "expected exactly one managed block begin marker in $file, got $begin_count"
  [[ "$end_count" == "1" ]] ||
    fail "expected exactly one managed block end marker in $file, got $end_count"
  ...
  [[ "$begin_line" -lt "$end_line" ]] ||
    fail "managed block markers out of order in $file"
}
...
assert_dir "examples/java-service-governance-demo"
assert_file "examples/java-service-governance-demo/README.md"
assert_executable "examples/java-service-governance-demo/run-demo.sh"
assert_file "examples/java-service-governance-demo/expected-output.txt"
assert_file "examples/java-service-governance-demo/fixture/pom.xml"
assert_file "examples/java-service-governance-demo/fixture/docs/AI_STATE.yml"
assert_file "examples/java-service-governance-demo/fixture/docs/tasks/T-010-demo.md"
assert_file "examples/java-service-governance-demo/fixture/src/main/java/com/example/order/OrderService.java"
assert_file "examples/java-service-governance-demo/fixture/src/main/java/com/example/order/OrderController.java"
assert_file "examples/java-service-governance-demo/fixture/src/main/resources/application.yml"
assert_file "examples/README.md"
assert_file ".github/workflows/test-kit.yml"
ci_workflow="$(cat .github/workflows/test-kit.yml)"
assert_contains "$ci_workflow" "git diff --check HEAD^ HEAD"
assert_contains "$ci_workflow" "bash -n installer/init.sh"
assert_contains "$ci_workflow" "bash -n scripts/test-kit.sh"
assert_contains "$ci_workflow" "bash -n scripts/readability-guard.sh"
assert_contains "$ci_workflow" "bash scripts/readability-guard.sh"
assert_contains "$ci_workflow" "bash scripts/test-kit.sh"
assert_file "CHANGELOG.md"
assert_file "scripts/readability-guard.sh"
assert_executable "scripts/readability-guard.sh"
...
assert_contains "$(cat README.md)" "examples/java-service-governance-demo/run-demo.sh"
...
assert_contains "$(cat examples/README.md)" "examples/java-service-governance-demo/run-demo.sh"
assert_file "docs/releases/v0.8.0.md"
assert_file "docs/tasks/T-011.md"
assert_file "docs/plans/T-011-plan.md"
assert_file "docs/RESUME_AND_INTERVIEW.md"
assert_contains "$(cat README.md)" "Resume and Interview Notes"
assert_contains "$(cat README.md)" "Java backend governance demo"
assert_contains "$(cat docs/POSITIONING.md)" "Java service governance demo"
assert_contains "$(cat docs/INTERVIEW_CN.md)" "Java Service Governance Demo"
assert_contains "$(cat docs/RESUME_AND_INTERVIEW.md)" "Resume Bullets"
assert_contains "$(cat docs/RESUME_AND_INTERVIEW.md)" "Interview Talk Track"
assert_contains "$(cat docs/RESUME_AND_INTERVIEW.md)" "Java backend governance demo"
...
assert_contains \
  "$(cat profiles/agent-adapters/root/CLAUDE.md)" \
...
assert_contains \
  "$(cat profiles/agent-adapters/root/.cursor/rules/vibecoding.mdc)" \
...
assert_contains \
  "$(cat profiles/agent-adapters/root/.github/copilot-instructions.md)" \
  "workflows/README.md"
...
readability_guard_output="$(bash scripts/readability-guard.sh)"
assert_contains "$readability_guard_output" "READABILITY_GUARD_PASS"
...
java_service_demo_output="$(bash examples/java-service-governance-demo/run-demo.sh)"
assert_contains "$java_service_demo_output" "DEMO_STEP fixture_prepared"
assert_contains "$java_service_demo_output" "DEMO_STEP service_change_allowed"
assert_contains "$java_service_demo_output" "DEMO_STEP unauthorized_file_detected"
assert_contains "$java_service_demo_output" "src/main/resources/application.yml"
assert_contains "$java_service_demo_output" "DEMO_STEP runtime_config_change_detected"
assert_contains "$java_service_demo_output" "overall_risk: HIGH"
assert_contains "$java_service_demo_output" "DEMO_STEP closeout_report_written"
assert_contains "$java_service_demo_output" "DEMO_PASS"
...
assert_contains \
  "$(cat "$MVP_TARGET/reports/vibecoding-init.md")" \
...
user-only
EOF
if (cd "$MVP_TARGET" && bash scripts/adapter-block.sh --check "$adapter_missing") \
  >"$TMP_DIR/adapter-missing.out" 2>&1; then
...
<!-- VIBECODING-KIT:END -->
EOF
if (cd "$MVP_TARGET" && bash scripts/adapter-block.sh --check "$adapter_duplicate_begin") \
  >"$TMP_DIR/adapter-duplicate-begin.out" 2>&1; then
...
<!-- VIBECODING-KIT:END -->
EOF
if (cd "$MVP_TARGET" && bash scripts/adapter-block.sh --check "$adapter_duplicate_end") \
  >"$TMP_DIR/adapter-duplicate-end.out" 2>&1; then
...
<!-- VIBECODING-KIT:BEGIN -->
EOF
if (cd "$MVP_TARGET" && bash scripts/adapter-block.sh --check "$adapter_malformed") \
  >"$TMP_DIR/adapter-malformed.out" 2>&1; then
...
adapter_update="$(
  cd "$MVP_TARGET" &&
    bash scripts/adapter-block.sh \
      --update \
      "$adapter_update_target" \
      "$adapter_update_template"
)"
assert_contains "$adapter_update" "ADAPTER_BLOCK_UPDATED"
cmp -s "$adapter_update_target" "$adapter_update_expected" ||
  fail "adapter-block update should preserve user content around managed block"
...
if (
  cd "$MVP_TARGET" &&
    bash scripts/adapter-block.sh \
      ...
      "$adapter_update_target" \
      "$adapter_missing"
) >"$TMP_DIR/adapter-update-bad-template.out" 2>&1; then
...
) >"$TMP_DIR/command-block.out" 2>&1; then
...
if (cd "$MVP_TARGET" && bash scripts/spec-lint.sh T-000) \
  >"$TMP_DIR/spec-lint-plan-status.out" 2>&1; then
  fail "spec-lint should reject unsupported plan status"
fi
assert_contains \
  "$(cat "$TMP_DIR/spec-lint-plan-status.out")" \
  "SPEC_LINT_FAIL: unsupported plan status"
...
if (cd "$MVP_TARGET" && bash scripts/spec-lint.sh T-000) \
  >"$TMP_DIR/spec-lint-step-status.out" 2>&1; then
  fail "spec-lint should reject unsupported step status"
fi
assert_contains \
  "$(cat "$TMP_DIR/spec-lint-step-status.out")" \
  "SPEC_LINT_FAIL: unsupported step status"
cp "$TMP_DIR/T-000-plan.status-good.md" "$MVP_TARGET/docs/plans/T-000-plan.md"
...
cp \
  "$MVP_TARGET/docs/specs/REQUIREMENTS_TEMPLATE.md" \
  "$MVP_TARGET/docs/specs/T-001-requirements.md"
sed -i \
  's/T-xxx/T-001/g; s/status: draft/status: approved/' \
  "$MVP_TARGET/docs/specs/T-001-requirements.md"
...
if (cd "$MVP_TARGET" && bash scripts/spec-lint.sh T-001) \
  >"$TMP_DIR/spec-lint-requirements-status.out" 2>&1; then
  fail "spec-lint should reject unsupported requirements status"
fi
assert_contains \
  "$(cat "$TMP_DIR/spec-lint-requirements-status.out")" \
  "SPEC_LINT_FAIL: unsupported requirements status"
...
if (cd "$MVP_TARGET" && bash scripts/spec-lint.sh T-001) \
  >"$TMP_DIR/spec-lint-design-status.out" 2>&1; then
  fail "spec-lint should reject unsupported design status"
fi
assert_contains \
  "$(cat "$TMP_DIR/spec-lint-design-status.out")" \
  "SPEC_LINT_FAIL: unsupported design status"
rm -f \
...
if (cd "$MVP_TARGET" && bash scripts/plan-step.sh T-000 S-001 --complete) \
  >"$TMP_DIR/plan-step-uncommitted.out" 2>&1; then
...
if (cd "$MVP_TARGET" && bash scripts/plan-guard.sh T-000 S-001) \
  >"$TMP_DIR/plan-unlocked.out" 2>&1; then
...
restore_plan_fixture
if (cd "$MVP_TARGET" && bash scripts/plan-step.sh T-000 S-999 --start) \
  >"$TMP_DIR/plan-step-command.out" 2>&1; then
...
restore_plan_fixture
sed -i \
  '0,/^- \*\*$/s//- docs\/AI_STATE.yml\n- docs\/plans\/T-000-plan.md/' \
  "$MVP_TARGET/docs/plans/T-000-plan.md"
set_ai_state_value \
  "$MVP_TARGET/docs/AI_STATE.yml" \
  "plan_hash" \
  "$(plan_hash "$MVP_TARGET/docs/plans/T-000-plan.md")"
printf 'drift\n' >"$MVP_TARGET/unplanned.txt"
if (cd "$MVP_TARGET" && bash scripts/plan-guard.sh T-000 S-001) \
  >"$TMP_DIR/plan-drift.out" 2>&1; then
...
if (cd "$MVP_TARGET" && bash scripts/plan-guard.sh T-000 S-001) \
  >"$TMP_DIR/plan-placeholder-hash.out" 2>&1; then
  fail "plan-guard should reject placeholder plan hash"
fi
assert_contains \
  "$(cat "$TMP_DIR/plan-placeholder-hash.out")" \
  "PLAN_GUARD_FAIL: missing or invalid plan_hash"
restore_plan_fixture
...
assert_contains \
  "$(cat "$MVP_TARGET/reports/ai-closeout/T-000.md")" \
...
assert_contains \
  "$(cat "$FULL_TARGET/docs/ai/PLAN_PROTOCOL.md")" \
...
assert_contains \
  "$(cat "$FULL_TARGET/.github/copilot-instructions.md")" \
...
assert_contains \
  "$(cat "$FULL_TARGET/.cursor/rules/vibecoding.mdc")" \
  "prompts/02-plan-locked-task.md"
...
assert_contains \
  "$(cat "$FULL_TARGET/reports/vibecoding-init.md")" \
  "bash scripts/task-closeout.sh T-000 --no-tests --write-report"
...
assert_contains \
  "$(cat "$FULL_TARGET/scripts/test-ai-guards.sh")" \
  "reports/ai-risk/T-000-risk-report.md"
